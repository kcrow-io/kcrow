...
OUTPUT_BASE_DIR=${OUTPUT_BASE_DIR:-${PROJECT_ROOT}/charts/kcrowdaemon}
...
manifests_gen() {
  ...
  controller-gen \
  crd object rbac:roleName="kcrow-controller-admin" \
  paths="${PWD}/${PROJECT_ROOT}/pkg/k8s/apis/kcrow.io/v1alpha1" \
...
manifests_verify() {
  ...
  if [ "$(ls -A ${OUTPUT_BASE_DIR}/crds)" ]; then
    cp ${OUTPUT_BASE_DIR}/crds/kcrow*  ${OUTPUT_TMP_DIR}/crds
  fi
