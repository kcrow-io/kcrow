...
func CreateVDevice(w WorkerInterface, env []string, devices []int) (VDeviceInfo, error) {
	invalidVDevice := VDeviceInfo{CardID: -1, DeviceID: -1, VdeviceID: -1}
	splitDevice, err := extractVpuParam(env)
	if err != nil {
	...
	if len(devices) != 1 || devices[0] < 0 || devices[0] >= hiAIMaxCardNum*hiAIMaxDeviceNum {
		klog.Errorf("invalid devices: %v", devices)
	...
	if err != nil || vdeviceID < 0 {
		klog.Errorf("cannot create vd or vdevice is wrong: %v %v", vdeviceID, err)
		return invalidVDevice, err
	}
	return VDeviceInfo{CardID: targetCardID, DeviceID: targetDeviceID, VdeviceID: vdeviceID}, nil
}

func extractVpuParam(env []string) (string, error) {
...
func GetProductType(w WorkerInterface) (string, error) {
	...
	if cardNum == 0 || err != nil {
		klog.Errorf("failed to get card list, err: %#v", err)
		return invalidType, err
	}
	for _, cardID := range cardList {
		devNum, err := GetDeviceNumInCard(cardID)
		if err != nil {
			klog.Infof("get device num by cardID(%d) failed, error: %#v", cardID, err)
			continue
		}
		if devNum == 0 {
			klog.Infof("not found device on card %d", cardID)
			continue
		}
		for devID := int32(0); devID < devNum; devID++ {
			productType, err := w.GetProductType(cardID, devID)
			if err != nil {
				klog.Infof("get product type by card %d deviceID %d failed, err: %#v", cardID, devID, err)
				continue
			}
...
func GetChipName() (string, error) {
	...
	for _, cardID := range cardList {
		devNum, err := GetDeviceNumInCard(cardID)
		if err != nil || devNum == 0 {
			klog.Warningf("get device num by cardID(%d) failed, error: %#v", cardID, err)
			continue
		}
		for devID := int32(0); devID < devNum; devID++ {
			chipInfo, err := dcWorker.GetChipInfo(cardID, devID)
			if err != nil {
				klog.Warningf("get chip info failed by cardID(%d), deviceID(%d), error: %#v", cardID, devID,
					err)
				continue
			}
			if !isValidChipInfo(chipInfo) {
				klog.Warningf("invalid chip info by cardID(%d), deviceID(%d), error: %#v", cardID, devID,
					err)
				continue
			}
